...
class Objectnet:
    """ObjectNet dataset (Paper: Barbu, A.; Mayo, D.; Alverio, J.; Luo, W.; Wang, C.;
    Gut- freund, D.; Tenenbaum, J.; and Katz, B. 2019. ObjectNet: A large-scale
    ...
    def load_data(self):
        ...
        def get_label_list_only_imagenet(data_pah):
            ...
            with open(
                data_pah+'mappings/imagenet_2012_label_humanreadable_mapping.json', "r"
            ) as read_file:
            ...
            with open(
                data_pah+'/mappings/objectnet_to_imagenet_1k.json', "r"
            ) as read_file:
        ...
        def get_label_list_not_imagenet(data_pah):
            ...
            with open(
                data_pah+'/mappings/folder_to_objectnet_label.json', "r"
            ) as read_file:
        ...
        def get_label_list_all_imagenet(data_path):
            ...
            return get_label_list_only_imagenet(data_path) +\
            get_label_list_not_imagenet(data_path)
        ...
        def get_str_label_dir_list(data_pah, label_list):
            ...
            for label in label_list:
                str_label = str(
                    data_pah+'images/'+str(objnetlabel_objnetfolder[label])+'/*'
                )
        ...
        def get_mapping_objnetlabel_imglabel(data_path):
            ...
            with open(
                data_path+'/mappings/imagenet_2012_label_humanreadable_mapping.json', "r"
            ) as read_file:
                label_human = json.load(read_file)
            human_label = {}
            for key,value in label_human.items():
            ...
            with open(
                data_path+'/mappings/objectnet_to_imagenet_1k.json', "r"
            ) as read_file:
                objnet_imgnethuman = json.load(read_file)
            ...
            for label_objnet in objnet_only_imagenet_labels:
                objnet_imglabel[label_objnet] = human_label[
                    objnet_imgnethuman[label_objnet]
                ]
        ...
        def get_mapping_objnetfolder_objnetlabel(data_path):
            ...
            with open(
                data_path+'/mappings/folder_to_objectnet_label.json', "r"
            ) as read_file:
                objnetfolder_objnetlabel = json.load(read_file)
        ...
        def get_tf_mapping_objnetfolder_imglabel(data_path):
            ...
            mapping_objnetfolder_objnetlabel = get_mapping_objnetfolder_objnetlabel(
                data_path
            )
            mapping_objnetlabel_imglabel = get_mapping_objnetlabel_imglabel(
                data_path
            )

            mapping_objnetfolder_imglabel = {}
            for folder in mapping_objnetfolder_objnetlabel.keys():
                if mapping_objnetfolder_objnetlabel[folder] in \
                list(mapping_objnetlabel_imglabel.keys()):
                    mapping_objnetfolder_imglabel[folder] =\
                    mapping_objnetlabel_imglabel[
        ...
        if self.subset in ['all','only_imagenet','not_imagenet']:
            ...
            raise ValueError('Error: subset must be one of the following all, \
            only_imagenet, not_imagenet')
        ...
        self.table_objnetfolder_imglabel = get_tf_mapping_objnetfolder_imglabel(
            self.data_path
        )
        list_ds = tf.data.Dataset.list_files(str_label_list)
        self.test_dataset = list_ds.map(
            process_path,
